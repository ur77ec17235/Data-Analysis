...
params = {
    'filter%5Blimit%5D': '20',
    'filter%5Bwhere%5D%5Btype%5D': 'course',
    'filter%5Bwhere%5D%5BisDisplayedOnMenu%5D': 'true',
    'filter%5Bwhere%5D%5BparentId%5D%5B%24not%5D%5B%24type%5D': 'objectId',
}

categories_id = []
for i in range(1, 30):
    params['page'] = i
    response = requests.get('https://medoo.io/api/v1/categories/', headers=headers, params=params)#, cookies=cookies)
    if response.status_code == 200:
        print('request success!!!')
        for record in response.json().get('data'):
            categories_id.append({'id': record.get('id')})
    time.sleep(random.randrange(3, 10))

df = pd.DataFrame(categories_id)
df.to_csv('categories_id_ncds.csv', index=False)